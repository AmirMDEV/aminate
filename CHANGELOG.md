...
This release collects the work made since the previous public release, v0.3.5. It includes the work that was planned for the never-published v0.3.6, plus the final v0.3.7 Pencil and reference-video fixes. The list below explains what changed in simple terms.

### Animator's Pencil: drawing, editing, and layers

- A single click now makes a small dot. It no longer makes a random long line.
- Holding the mouse button and dragging draws continuously in real time. The last part of a stroke is kept even when the reference video or Maya briefly hitches.
- The Start Drawing button turns bright green while drawing is active, so it is easy to see which mode is on.
- Changing pencil size, opacity, or other settings safely re-arms the drawing tools instead of making drawing or marquee selection stop.
- Drawing and marquee selection now keep separate mouse contexts. Switching tools no longer leaves the old tool partly active.
- The Pencil view has a clear Active Drawing area, quick-start help, layer controls, layer opacity, visibility, locked-layer handling, renaming, and a saved camera-space position.
- Pencil marks stay attached to the camera they were drawn for. They no longer wander when the Maya perspective view changes.
- The eraser works while you drag, can erase more than the newest line, and finds marks on the selected layer and current frame.
- The eraser can remove the correct part of a stroke instead of only deleting the last complete mark. Whole-stroke erase still works when the selected text covers the whole stroke.
- Brush radius, eraser radius, hotkeys, RGB colour controls, swatches, menus, and shape buttons now use the same active settings.
- The shape library can make boxes, circles, arrows, and other simple marks with camera-correct corners.
- Marquee selection is drawn over the viewport, not off to the side. A full mark can be moved, resized, and rescaled.
- A marquee can also select only part of a mark. The selected part can be moved without deleting the rest of the drawing.
- The temporary yellow transform box is removed when the transform is finished, so it does not become a permanent mark.
- Marquee and shape boxes use the ordered camera corners. They no longer appear rotated or skewed just because the viewport is in perspective.
- Missed mouse-up callbacks and short input gaps are recovered, so a stroke does not end early.
- Tooltips, navigation fallback, action groups, and the tool picker make it easier to find and use the Pencil commands.
- Pencil layers keep their visibility, opacity, and frame data when you switch layers or cameras.
- One-frame drawings, onion-skin support, Camera Notes, saved views, undo, and redraw refreshes are kept in the same camera-space workflow.

### Reference video and camera workflow

- Reference videos can be attached as normal videos or lighter proxy clips. Short clips, MKV files, and audio settings are handled more safely.
- The reference viewer can fill the main view, show a smaller corner view, or float as a freeform window.
- The viewer has controls for size, start frame, opacity, audio, close/reopen, and saved-view names.
- Dragging the title bar changes the viewer from its old corner position to freeform mode. It can be moved to another monitor and made large there.
- The reference image plane is private to the reference viewer camera. It is not shown in the normal Perspective or other Maya viewports.
- Changing the main Maya camera no longer makes the viewer lose the selected layer's draw-over marks.
- Attaching a video can switch the active main viewport to the same Pencil View camera as the reference viewer, so you can draw over the large video view.
- Restore Previous Camera returns the main viewport to the user's earlier Perspective or other camera. The user's camera is never permanently replaced.
- Reference viewer image planes, draw-overs, and camera data are cleaned up safely when a viewer closes or a scene is reopened.
- Package Scene keeps reference paths portable when a scene is moved to another machine.

### Aminate window, shelf, and tutorial

- The product name is now Aminate everywhere. Old Maya Workflow Anim Tools wording was removed from the user-facing flow.
- The Aminate shelf button keeps the full mascot icon visible and uses a readable label while keeping the same launch action.
- The main Aminate window follows Maya's native workbench style. Search, Tool Picker, Jump To, and grouped tool sections make the large toolset easier to browse.
- The visual system uses Maya Graphite, Studio Contrast, and Toolkit Spectrum colours so the window stays clear without looking like a separate app.
- The fixed bottom Toolkit Bar and Game Mode controls keep their icons and labels aligned instead of clipping or drifting.
- Help, coach text, the footer, Donate, and followamir links are easier to reach without covering the tools.
- The layout responds to narrow and wide windows, including small 360-pixel widths and large monitor layouts.
- The offline tutorial now covers all 23 tools with picture-first steps, search, categories, FAQ answers, theme controls, and local media.
- tutorial.html is available at the project root for quick local opening. The tutorial is packaged as its own offline ZIP and is not placed inside the main Aminate installer ZIP.

### Animation and rig tools

- Rig Scale is simpler to use and bakes keys at the original key times, including fractional times. It does not add a key on every frame by default.
- The original-key-time rule is now the default for every Aminate baking system.
- Controls Retargeter keeps the first source pose exactly and transfers sparse key times without adding unwanted keys.
- The Control Picker updates its buttons from the current rig and gives clear feedback when a control is missing.
- Character Skinning offers Track A and Track B flows with safer selection and transfer steps.
- Dynamic Pivot uses the chosen frame range and keeps the current frame safe while it works.
- Dynamic Parenting and Universal IK/FK have clearer setup, cleanup, and current-frame behaviour.
- Surface Contact and Hand/Foot Hold include visible setup and bake feedback.
- Selected-animation FBX export exports only what the user selected and keeps the result reviewable.
- Smear Frames can create static and Unreal-friendly morph-style outputs without changing the original rig.
- Animation Layers controls, Retime Every N, Key Full Blocking Poses, and 1080p Playblast are easier to run from the same tool window.

### History, performance, and Maya safety

- Game Mode asks before enabling Auto History, so the user can choose whether Maya should record extra history.
- History Timeline can jump safely, show branches, group related actions, and restore a saved point without deleting the scene.
- Heavy rigs and saved-view panels do less work while idle, so the Aminate window opens and responds faster.
- Controller panels load only when needed. The Qt6Core dock path is guarded so a missing dock widget does not crash startup.
- Closing Aminate hides its UI and releases its own state. It does not purge Maya modules, delete workspaces, or remove the user's scene data.
- The right dock is created once, with a floating fallback when Maya cannot dock it normally. The bottom bar also cleans itself up when the window closes.
- Startup, update, installer, backup, and crash paths are more careful about unsaved scenes and the one existing Maya session.
- Install and update checks show which version is actually running, and they keep unrelated Maya tools untouched.

### Packaging and release

- Aminate now has a self-contained, versioned drag-and-drop installer for Maya, plus a legacy installer fallback.
- Dropping the same installer into the same Maya session again is safe and does not duplicate shelves, modules, or startup entries.
- The installer, manifest, source, and installed runtime report matching versions and file hashes.
- The main Aminate ZIP contains the Maya installer and tool files. The offline tutorial is a separate ZIP asset so it can be downloaded on its own.
- The public repository is now `AmirMDEV/aminate`, with the release package and tutorial asset clearly named.
- The Pencil and reference-video camera workflows are marked as beta testing features in the notes. The release version itself is stable and is not labelled beta.
